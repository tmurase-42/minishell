...
/*   Updated: 2021/09/20 14:04:11 by tdofuku          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_exit_status(t_cmd *cmd, char **message, t_bool *exit_flag)
{
	...
	if (cmd->args->next)
		...
		first_args = NULL;
	if (cmd->argc == 3 && ft_strncmp(first_args, "--",
			ft_strlen(first_args)) == 0 && cmd->args->next->next
		&& is_strdigit(cmd->args->next->next->data))
